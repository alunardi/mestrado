...
def calculaIg(num):
    ...
    for key in ngrama_total:
        ...
        if key != '':
            ...
            ganho  = -entropia_classe + (ngrama_total[key][5]/numero_review)*entropia_termo + ((numero_review-ngrama_total[key][5])/numero_review)*entropia_nao_termo
            if ganho < 0.0:
                ganho = ganho*(-1)
            ig_final[key] = ganho
    import operator    
    ...
    print sorted_x
    salvaArquivo('Bigramas/ig_%s_bi.txt' % num,num,sorted_x)