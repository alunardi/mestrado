...
def calculaChi(num):
    ...
    for key in unigrama_total:
        ...
        if key != '':
            for j in range(5):
                n = 7500
                a = float(unigrama_total[key][j])                
                b = float(unigrama_total[key][5]) - a
                e = total_review[j] - a                
                d = n - (total_review[j] + float(unigrama_total[key][5]) - a)
    ...
    salvaArquivo('Unigramas/chi_%s_uni.txt' % num,num,sorted_x)